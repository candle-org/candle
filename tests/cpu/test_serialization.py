...
import candle as mt
import candle.serialization as ser
...
def test_serialization_does_not_import_torch_runtime(monkeypatch):
    ...
    def guarded_import(name, *args, **kwargs):
        if name == "torch" or name.startswith("torch."):
            raise AssertionError("torch import is not allowed in candle serialization")
...
def test_torch_save_then_candle_load_state_dict(tmp_path):
...
def test_torch_legacy_save_then_candle_load_state_dict(tmp_path):
    model = torch.nn.Linear(4, 3)
...
def test_candle_save_then_torch_load_state_dict(tmp_path):
    ...
    path = tmp_path / "candle_state.pth"
    mt.save(state, path)
...
def test_torch_save_then_candle_load_bytesio_roundtrip():
    model = torch.nn.Linear(4, 3)
...
def test_candle_save_then_torch_load_bytesio_roundtrip():
    model = nn.Linear(4, 3)
...
def test_candle_save_load_nested_common_checkpoint(tmp_path):
    model = nn.Linear(4, 3)
    ckpt = {
...
def test_candle_save_load_bytesio_roundtrip():
    model = nn.Linear(4, 3)
...
def test_torch_save_optimizer_state_then_candle_load(tmp_path):
    model = torch.nn.Linear(4, 3)
...
def test_save_unsupported_dtype_uint16_raises(tmp_path):
    ...
    path = tmp_path / "candle_uint16.pth"
...
def test_candle_roundtrip_preserves_storage_aliasing():
...
def test_torch_to_candle_preserves_storage_aliasing(tmp_path):
    t = torch.arange(6.0)
    obj = {"a": t[:4], "b": t[2:]}
...
def test_torch_to_candle_preserves_non_contiguous_stride(tmp_path):
...
def test_candle_to_torch_preserves_non_contiguous_stride(tmp_path):
    t = mt.arange(0, 12).reshape((3, 4)).transpose(0, 1)
    path = tmp_path / "candle_noncontig.pth"
    mt.save({"x": t}, path)
...
def test_torch_to_candle_dtype_matrix(tmp_path, dtype):
...
def test_candle_to_torch_dtype_matrix(tmp_path, dtype):
    if dtype == mt.bool:
    ...
    path = tmp_path / f"candle_dtype_{dtype.name}.pth"
    mt.save({"x": t}, path)
...
def test_candle_save_unsupported_unsigned_dtypes_raise(tmp_path, dtype):
    t = mt.tensor([1, 2, 3], dtype=dtype)
    path = tmp_path / f"candle_{dtype.name}_unsupported.pth"
