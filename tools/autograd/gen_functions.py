...
# methods call.  In the .pyx these resolve through the cdef object module globals.
# We strip the Python import preamble (first ~17 lines of _HEADER) and keep only
# the helper function definitions (from _inverse_permutation onwards).  The
# _inverse_permutation def in _PYX_HEADER above is a re-declaration; we skip it
# in the helpers slice.


def _get_pyx_helpers_block() -> str:
    """Extract the helper function block from _HEADER (skip import preamble).

    Applies the following fixups for the Cython helper block:
    - Replaces `seq[-1]` with `seq[len(seq) - 1]` for wraparound=False safety.
    - Rewrites bare function names to their cached module-level refs so the
      helper bodies resolve through the cdef object globals set by _ensure_refs()
      rather than re-importing on every call:
        redispatch(             -> _redispatch(
        reduce_grad(            -> _reduce_grad(
        current_dispatch_keyset( -> _current_dispatch_keyset(
      Names that already start with '_' (e.g. _scalar_tensor_like) are left as-is
      because they already refer to the cached name.
    """
    import re
    lines = _HEADER.splitlines(keepends=True)
    # Find the first line that starts with 'def _sqrt_backward_helper'
    # to skip the import preamble and the _inverse_permutation def which
    # is already emitted in _PYX_HEADER.
    start = 0
    for i, line in enumerate(lines):
        if line.startswith('def _sqrt_backward_helper'):
            start = i
            break
    block = ''.join(lines[start:])
    # Fix negative indexing for wraparound=False: `name[-1]` -> `name[len(name) - 1]`
    # These patterns occur in *extra_and_keyset and *args variadic helpers.
    block = re.sub(
        r'\b(extra_and_keyset)\[-1\]',
        r'\1[len(\1) - 1]',
        block,
    )
    block = re.sub(
    ...
    # Rewrite bare function names to cached module-level refs.
    # Use negative lookbehind (?<!_) so already-prefixed names like
    # _redispatch are not double-prefixed to __redispatch.
    block = re.sub(r'(?<!_)\bredispatch\(', '_redispatch(', block)
    block = re.sub(r'(?<!_)\breduce_grad\(', '_reduce_grad(', block)
    block = re.sub(r'(?<!_)\bcurrent_dispatch_keyset\(', '_current_dispatch_keyset(', block)
    return block
...
def _gen_one_node_pyx(info: DifferentiabilityInfo) -> str:
    ...
    lines: list = []
    lines.append(f"\nclass {cls_name}(_Node):")

    # __init__
    lines.append("    def __init__(self, inputs, *, raw_keyset=None, active_keyset=None):")
    lines.append(f"        _ensure_refs()")
    lines.append(f"        super().__init__(None, inputs, name={cls_name!r})")
    lines.append("        self._raw_keyset = raw_keyset")
    lines.append("        self._active_keyset = active_keyset")
    for name in saved_inputs:
