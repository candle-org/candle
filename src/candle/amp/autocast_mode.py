...
def _set_npu_functional_autocast_active(active):
    ...
    if _NPU_FUNCTIONAL_AUTOCAST_SETTER is None:
        try:
            from candle._C._tensor_api import cy_set_npu_autocast_active  # pylint: disable=import-error,no-name-in-module
        except ImportError:
...
def _cuda_available():
    try:
        import importlib
        cuda_api = importlib.import_module("candle.cuda")
    except Exception:
        return False
