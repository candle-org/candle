...
def _layer_norm_backward(grad, _a, saved_a, keyset, args, kwargs, backward_data=None):
    ...
    with _grad_context(keyset):
        ...
        grad_bias = None
        if weight is not None and getattr(weight, "requires_grad", False):
            # grad_weight = sum over batch dims of (grad * x_hat)
...
for _entry in (
    # In-place ops (not migrated to derivatives.yaml)
    ...
    # Special dispatch patterns (not migrated)
    ...
    # Custom wrapper ops (multi-tensor, special patterns)
