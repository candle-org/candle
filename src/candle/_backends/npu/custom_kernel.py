"""AscendC custom kernel integration for candle.
...
- tensor_to_acl() — creates ACL tensor descriptor from a candle Tensor
...
def tensor_to_acl(t):
    """Create an ACL tensor descriptor from a candle Tensor.

    Args:
        t: A candle Tensor (must be on NPU device with valid storage).
