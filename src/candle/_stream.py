...
class PyTorchStreamReader:
    ...
    def _init(self) -> None:
        ...
        # Open the zip via a BytesIO wrapper so zipfile can parse it
        all_data = self._read_raw(0, self._raw_size)
        self._data_buf = all_data
        self._zf = zipfile.ZipFile(io.BytesIO(all_data), 'r')
...
class PyTorchStreamWriter:
    ...
    def _writeSerializationId(self) -> None:
        ...
        for record_name in self._files_written:
            h = hash(record_name) & 0xFFFFFFFFFFFFFFFF
