...
- If multiple CANN versions are installed in the environment, always use the newest CANN version.
- Before running NPU diagnostics or tests, verify that all CANN-related paths (toolkit, runtime libraries, OPP/model directories) resolve to the newest installed version.
- PR markdown formatting rule: when using `gh pr create` or `gh pr edit`, never pass literal `\\n` sequences in `--body`; always provide real newlines via `--body-file` (or a heredoc file) and verify rendered PR body after submission.
